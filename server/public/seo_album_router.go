...
func (ar *AlbumRouter) handleAlbumPage(w http.ResponseWriter, r *http.Request) {
	...
	descText := stripHTML(album.Description)
	if descText == "" {
		descText = fmt.Sprintf("Альбом «%s» — %s. %d песен. Слушайте онлайн на Qirim.Online.", album.Name, album.AlbumArtist, len(trackItems))
	}
	metaDesc := descText
	if len(metaDesc) > 300 {
